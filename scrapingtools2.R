...
count_skills_loc <- function(df){
 ...
 test <- test[order(-test$skills),]
 test <- df[1:5,]
    
return(df) 
    
 # df <- separate(df, Skills, c("S1", "S2", "S3", "S4", "S5", "S6"), sep=", ", fill="right")
#  df <- select(df,S1,S2,S3,S4,S5,S6,Location)
# df <- gather(df, "Location", "skills", -Location)
#  df <- filter(df, skills != "")
#  df <- df %>% 
#    count(skills) %>% 
#    filter(!is.na(skills))

...
leaflet_points <- function(df,df2, coords){
  ...
  locations_df <- df %>% 
    ...
    separate(Location, into=c("State","City"), sep = " - ")
  
  locations_df$Location <- paste(locations_df$City,locations_df$State,sep=", ")
...
count_wages <- function(df){
  dswages <- df %>%
    count(Salary) %>%
    filter(Salary != "null" & Salary != " " & Salary != "" & Salary < 700000 & Salary > 15000)
